...
export class ViewingService implements ViewingServiceContract {
  ...
  async create(actor: AuthenticatedUser, input: CreateViewingInput) {
    ...
    try {
      return toResponse(
        await this.repository.create(actor.agencyId, actor.id, input),
      );
    } catch (error) {
  ...
  async calendar(actor: AuthenticatedUser, query: CalendarQuery) {
    return (
      await this.repository.calendar(
        actor.agencyId,
  ...
  async update(
    ...
    if (input.status !== undefined) {
      assertViewingTransition(current.status, input.status);
    }
  ...
  async remove(actor: AuthenticatedUser, id: string) {
    ...
    if (!(await this.repository.softDelete(actor.agencyId, id, this.clock()))) {
      throw viewingNotFound();
    }
  }

  private async requireViewing(actor: AuthenticatedUser, id: string) {
    const viewing = await this.repository.findById(
      actor.agencyId,
      id,
      permittedAgent(actor),
    );
  ...
  private async validateRelations(
    ...
    const valid = await this.repository.validateRelations(
      agencyId,
      value.propertyId,
      value.clientId,
      value.dealId,
    ...
    if (!valid.creator)
      throw new AppError("Viewing creator is unavailable", 400);
  }
}

const validateMergedState = (value: {
  ...
  if (value.status === ViewingStatus.COMPLETED && !value.outcome?.trim()) {
    throw new AppError("Outcome is required for completed viewings", 400);
  }
  if (
...
const requireManager = (actor: AuthenticatedUser) => {
...
const mapConflict = (error: unknown) =>
  error instanceof ViewingScheduleConflictError
    ? new AppError("Assigned agent has an overlapping viewing", 409)
    : error;
