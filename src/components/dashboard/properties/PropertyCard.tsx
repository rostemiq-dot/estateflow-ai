    ...
    <article className="group min-w-0 max-w-full overflow-visible rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-md">
      <div className="relative h-44 overflow-visible rounded-t-2xl bg-gradient-to-br from-slate-950 via-slate-800 to-amber-700">
        ...
        <div className="absolute inset-x-4 bottom-4 flex items-end justify-between gap-3">
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-slate-950 shadow-sm backdrop-blur">
        ...
        <div ref={menuRef} className="absolute right-3 top-3 z-20">
          <button
            ...
            className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl bg-white/95 text-slate-700 shadow-sm backdrop-blur transition hover:bg-white hover:text-slate-950 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200"
          >
          ...
          {isMenuOpen && (
            <div
              role="menu"
              className="absolute right-0 top-12 w-48 overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl"
            >
              <button
                role="menuitem"
                type="button"
                onClick={() => runAction(onView)}
                className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-semibold text-slate-700 transition hover:bg-slate-50 hover:text-slate-950"
              >
              ...
              <button
                ...
                className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-semibold text-rose-600 transition hover:bg-rose-50"
              >
      ...
      <div className="overflow-hidden rounded-b-2xl p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <span
            className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-bold ${statusStyles[property.status]}`}
          >
            {property.status}
          </span>
        </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3 border-y border-slate-100 py-4 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Baths</p>
          </div>
          <div>
            <p className="text-sm font-bold text-slate-950">
              {property.areaSqm}m²
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm font-bold text-emerald-600">
              {matchCount > 0
                ? `${bestMatchScore}% best · ${matchCount} ${
                    matchCount === 1 ? "client" : "clients"
                  }`
                : "No saved client match"}
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={onView}
            className="min-h-11 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-100"
          >
            Open
          </button>
        </div>
