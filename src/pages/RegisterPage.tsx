...
export function RegisterPage(){
  ...
  async function submit(e:FormEvent){
    ...
    if(!turnstileSiteKey)return setError("Security verification is not configured for this deployment.");
  ...
  return <AuthLayout title="Create your account" subtitle="Start managing your real estate business"><form onSubmit={submit} className="space-y-4"><Field label="Full name" value={name} onChange={setName}/><Field label="Email" type="email" value={email} onChange={setEmail}/><Field label="Password" type="password" value={password} onChange={setPassword}/><Field label="Confirm password" type="password" value={confirm} onChange={setConfirm}/>{turnstileSiteKey&&<TurnstileWidget siteKey={turnstileSiteKey} resetKey={captchaResetKey} onToken={handleCaptchaToken}/>} {error&&<p role="alert" className="text-sm text-rose-600">{error}</p>}{message&&<p role="status" className="text-sm text-emerald-600">{message}</p>}<button disabled={busy||!captchaToken} className="w-full rounded-xl bg-slate-950 py-3 font-bold text-white disabled:opacity-50">{busy?"Creating account…":"Create account"}</button><p className="text-center text-sm text-slate-500">Already registered? <Link className="font-bold text-amber-700" to="/login">Sign in</Link></p></form></AuthLayout>
}
